...
```bash
python3 simulate_bot_loss.py  #Linux/macOS
python simulate_bot_loss.py  #Win
```
Если ping_status = loss, создается запись в базе, а если ping_status меняет на up, то идет смена статуса с open на resolved для этой записи.
